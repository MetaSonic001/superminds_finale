  ...
  export async function GET(request: Request) {
    ...
    try {
      const response = await fetch(
        `http://192.168.2.239:8000/data?keywords=${keywords}&reddit=${includeReddit}&youtube=${includeYoutube}&facebook=${includeFacebook}`
      )
  ...
  function analyzeAdCreatives(ads: Array<{ ad_creative_bodies: string[] }>) {
    return ads.flatMap(ad => ad.ad_creative_bodies)
      .map(body => body.split('\n'))
      .flat()
  ...
  function generateMarketingInsights(data: DataResponse) {
    ...
    if (data.youtube_videos.length > 0) {
      insights.push({
        ...
        recommendation: 'Consider creating content that compares different fitness solutions'
      })
    }
    ...
    if (data.facebook_ads.length > 0) {
      insights.push({
        category: 'Ad Copy',
        insight: 'Successful ads emphasize quick results and easy-to-follow programs',
